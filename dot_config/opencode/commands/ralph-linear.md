---
description: Implement the first open checklist item from a Linear issue, commit it, then mark it complete
---

Implement exactly one sequential checklist item from a Linear issue.

The only argument is the Linear issue identifier, for example `NIT-123`:

`$ARGUMENTS`

Workflow:

1. Validate that `$ARGUMENTS` contains exactly one Linear issue identifier. Accept either hyphenated (`HE-1234`) or compact (`HE1234`) form, normalizing to the canonical hyphenated identifier when calling Linear tools. If it is missing or ambiguous, print `=== BLOCKED ===`, ask for the identifier, and stop.
